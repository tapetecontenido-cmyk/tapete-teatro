          ...
          {reserva.estado === 'pendiente' && (
            <div>
              ...
              <textarea
                ...
                placeholder="Ej: Pago verificado correctamente. Por favor presentar identificación en taquilla."
                rows={3}
...
export default function AdminReservas() {
  ...
  const confirmarReserva = async (reservaId, nota) => {
    try {
      ...
      // Notificar al backend para enviar email
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {[
          ...
          <button
            ...
            className={clsx(
              'px-4 py-2 rounded-xl text-sm font-heading font-bold transition-all',
              filtroEstado === key
                ? 'bg-azul text-white shadow-brand'
                : 'bg-white text-gray-600 border border-gray-200 hover:border-azul hover:text-azul'
            )}
          >
            {label}
            <span className={clsx(
              'ml-2 px-1.5 py-0.5 rounded-full text-xs',
              filtroEstado === key ? 'bg-white/20' : 'bg-gray-100'
            )}>
              {contadores[key]}
            </span>
          </button>
      ...
      <div className="relative mb-5">
        ...
        <input
          ...
          placeholder="Buscar por nombre, email, ID o referencia..."
  ...
  {r.obraNombre || '—'}
                    ...
                    <td className="px-5 py-3 hidden lg:table-cell">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {r.asientos?.length > 3 && (
                          <span className="text-xs text-gray-400">+{r.asientos.length - 3}</span>
                        )}
                      </div>
                    </td>
  ...
  {r.metodoPago?.replace('_', ' ')}
                    ...
                    <td className="px-5 py-3 text-center hidden lg:table-cell text-xs text-gray-400">
                      {r.creadoEn && format(
                        r.creadoEn.toDate?.() || new Date(), 'dd/MM/yy'
                      )}
                    </td>
                    <td className="px-5 py-3 text-center">
                      <button
