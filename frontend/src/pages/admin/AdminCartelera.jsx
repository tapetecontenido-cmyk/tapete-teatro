...
import { Plus, Edit2, Trash2, X, Upload, CheckCircle, Calendar } from 'lucide-react';
...
export default function AdminCartelera() {
  ...
  const [form, setForm] = useState({ nombre: '', genero: '', descripcion: '', director: '', reparto: '', duracion: '' });
  ...
  const abrirNuevo = () => {
    setEditando(null);
    setForm({ nombre: '', genero: '', descripcion: '', director: '', reparto: '', duracion: '' });
    setPoster(null); setModal(true);
  };

  const abrirEditar = (obra) => {
    setEditando(obra);
    setForm({
      nombre: obra.nombre, genero: obra.genero || '', descripcion: obra.descripcion || '',
      director: obra.director || '', reparto: obra.reparto || '', duracion: obra.duracion || '',
    });
    setPoster(null); setModal(true);
  };
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const data = {
        ...
        posterUrl, activo: true, actualizadoEn: serverTimestamp(),
      };
          ...
          <div key={obra.id} className="card overflow-hidden">
            ...
            <div className="p-4">
              ...
              <div className="grid grid-cols-2 gap-2 mt-4">
                ...
                <button onClick={() => moverAPapelera(obra)} className="flex items-center justify-center gap-1 py-2 rounded-lg border border-gray-200 text-xs font-heading font-bold text-gray-600 hover:border-red-400 hover:text-red-500 transition-colors"><Trash2 size={13} /> Eliminar</button>
              </div>
            </div>
          </div>
        ))}
              ...
              {papeleraObras.map(item => (
                <div key={item.id} className="flex items-center gap-4 p-4 rounded-xl bg-orange-50 border border-orange-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">{item.genero}</p>
